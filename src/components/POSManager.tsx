    ...
    <div className="flex flex-col lg:flex-row gap-6 max-w-[1600px] mx-auto pb-10">
      ...
      <div className="flex-1 flex flex-col space-y-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3">
          ...
          {/* Category Filter Cards Grid (Boxy, Colorful, 3-4 per row) */}
          <div className="pt-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              <button
                ...
                className={`px-3 py-2.5 rounded-xl border transition-all text-left flex items-center justify-between h-10 ${
                  selectedCategoryId === 'all'
                    ? 'bg-slate-900 text-white border-slate-900 shadow-md ring-2 ring-slate-400/30 font-bold'
                    : 'bg-slate-800 text-slate-100 border-slate-700 hover:bg-slate-900 font-semibold shadow-xs'
                }`}
                id="pos-cat-all"
              >
                <div className="flex items-center space-x-1.5 min-w-0 pr-1">
                  <Sparkles className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                  <span className="font-black text-[11px] tracking-tight truncate text-white">All Treatments</span>
                </div>
                <span
                  className={`text-[9px] px-1.5 py-0.5 rounded-full font-mono font-bold shrink-0 ${
                    selectedCategoryId === 'all' ? 'bg-white/20 text-white' : 'bg-slate-700 text-slate-200'
                  }`}
                >
              ...
              {categories.map((cat, idx) => {
                ...
                // Fallback palette colors if cat.color is missing or default
                const fallbackColors = ['#0d9488', '#e11d48', '#8b5cf6', '#2563eb', '#059669', '#d97706', '#0284c7', '#c026d3'];
                const colorHex = cat.color || fallbackColors[idx % fallbackColors.length];
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-2.5 rounded-xl border transition-all text-left flex items-center justify-between h-10 ${
                      isSelected
                        ? 'text-white shadow-md ring-2 ring-offset-1 font-bold'
                  ...
                  >
                    <span 
                      ...
                      className="font-extrabold text-[11px] tracking-tight truncate capitalize pr-1 dark:text-slate-100"
                    >
                      {cat.name}
                    </span>
                    <span
                      style={{
                        backgroundColor: isSelected ? 'rgba(255,255,255,0.25)' : colorHex,
                        color: '#ffffff',
                      }}
                      className="text-[9px] px-1.5 py-0.5 rounded-full font-mono font-bold shadow-xs shrink-0"
                    >
                      {catServiceCount}
                    </span>
                  </button>
        ...
        {/* Service Cards Grid (Ultra Compact 4 Columns) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          {filteredServices.length === 0 ? (
            <div className="col-span-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 text-center space-y-2">
              <ShoppingCart className="w-7 h-7 text-slate-400 mx-auto" />
            ...
            filteredServices.map((service) => {
              ...
              return (
                <div
                  ...
                  className={`group relative rounded-xl p-2.5 transition-all duration-150 cursor-pointer flex flex-col justify-between border shadow-sm hover:shadow-md ${
                    inCart
                      ? 'border-blue-600 ring-2 ring-blue-600/30 bg-blue-50/50 dark:bg-blue-950/40 shadow-md'
                      : 'border-slate-200/90 dark:border-slate-700/80 bg-white dark:bg-slate-900 hover:border-blue-300 dark:hover:border-slate-600'
                  }`}
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    <div className="flex items-center justify-between gap-1">
                      <span
                        className="text-[8px] px-1.5 py-0.2 rounded-md font-bold uppercase tracking-wider text-white truncate max-w-[75px]"
                        style={{ backgroundColor: cat?.color || '#2563eb' }}
                      >
                        {cat?.name || 'Service'}
                      </span>
                      
                      <span className="text-[9px] text-slate-500 font-mono flex items-center gap-0.5 flex-shrink-0">
                        <Clock className="w-2.5 h-2.5 text-blue-600" />
                        {service.durationMinutes}m
                      </span>
                    </div>

                    <h4 className="font-bold text-[11px] text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-1 pt-0.5 leading-tight">
                      {service.name}
                    </h4>

                    <p className="text-[9px] text-slate-400 line-clamp-1 leading-none">
                  ...
                  <div className="flex items-center justify-between pt-1.5 mt-1.5 border-t border-slate-100 dark:border-slate-800">
                    <div>
                      <span className="text-[10px] font-black text-blue-700 dark:text-blue-400 font-mono leading-none block">
                    ...
                    <button
                      ...
                      className={`px-1.5 py-0.5 rounded-md text-[9px] font-extrabold transition-all flex items-center space-x-0.5 ${
                        inCart
                          ? 'bg-blue-600 text-white shadow-xs'
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 group-hover:bg-blue-600 group-hover:text-white'
                      }`}
                    >
                      {inCart ? (
                        <>
